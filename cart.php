...
<?php if ($items): ?>
    <form action="cart-action.php" method="post">
        ...
        <?php foreach (
            $items
            as $item
        ): ?><tr><td><div class="d-flex align-items-center gap-3"><img src="<?= htmlspecialchars(
    $item["image"],
) ?>" alt="" width="70" height="60" style="object-fit:cover"><strong><?= htmlspecialchars(
    $item["name"],
) ?></strong></div></td><td>Rs. <?= number_format(
    $item["price"],
    2,
) ?></td><td><input class="form-control" type="number" name="quantity[<?= $item[
    "id"
] ?>]" value="<?= $item["quantity"] ?>" min="0" max="<?= max(
    1,
    $item["stock"],
) ?>" data-cart-quantity data-price="<?= htmlspecialchars(
    (string) $item["price"],
    ENT_QUOTES,
    "UTF-8",
