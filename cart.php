...
<?php if ($items): ?>
    <form action="cart-action.php" method="post">
        ...
        <?php foreach ($items as $item): ?><tr><td><div class="d-flex align-items-center gap-3"><img src="<?= htmlspecialchars($item['image']); ?>" alt="" width="70" height="60" style="object-fit:cover"><strong><?= htmlspecialchars($item['name']); ?></strong></div></td><td>Rs. <?= number_format($item['price'], 2); ?></td><td><input class="form-control" type="number" name="quantity[<?= $item['id']; ?>]" value="<?= $item['quantity']; ?>" min="0" max="<?= max(1, $item['stock']); ?>" style="max-width:90px"></td><td>Rs. <?= number_format($item['subtotal'], 2); ?></td><td><button class="btn btn-outline-danger btn-sm" name="action" value="remove" formaction="cart-action.php" formmethod="post" onclick="this.form.product_id.value='<?= $item['id']; ?>'">Remove</button></td></tr><?php endforeach; ?>
        </tbody></table></div>
        <input type="hidden" name="product_id" value="">
        <div class="d-flex flex-wrap justify-content-between gap-3 align-items-center"><button class="btn btn-outline-dark" type="submit">Update Cart</button><div class="text-end"><h4>Total: <span class="text-warning">Rs. <?= number_format($total, 2); ?></span></h4><a class="btn btn-warning" href="contact.php">Send an Inquiry</a></div></div>
    </form>
<?php else: ?>
