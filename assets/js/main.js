document.addEventListener("DOMContentLoaded", () => {
  ...
  const savedScroll = Number.parseInt(
  ...
  document
    .querySelectorAll('form[action="cart-action.php"]')
    .forEach((form) => {
  ...
  const updateCartAmounts = () => {
    ...
    cartQuantities.forEach((quantityInput) => {
      ...
      quantityInput
        .closest("tr")
        .querySelector("[data-cart-subtotal]").textContent =
  ...
  cartQuantities.forEach((quantityInput) =>
    quantityInput.addEventListener("input", updateCartAmounts),
  );
      ...
      searchTimer = window.setTimeout(async () => {
        ...
        try {
          const response = await fetch(
            `search-suggestions.php?q=${encodeURIComponent(query)}`,
